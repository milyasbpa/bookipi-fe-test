...
vi.mock('@/core/api/generated/quizzes/quizzes', () => ({
  useUpdateQuiz: vi.fn((options) => ({
    mutate: vi.fn((data) => {
      options?.mutation?.onSuccess?.({ id: 1, title: 'Test Quiz', description: 'Test', timeLimitSeconds: 300, isPublished: true });
    }),
    isPending: false,
  })),
}));
...
describe('useUpdateQuiz', () => {
  ...
  it('shows error toast on failure', async () => {
    ...
    vi.mocked(useUpdateQuizGenerated.useUpdateQuiz).mockImplementationOnce((options: any) => ({
      mutate: vi.fn((data) => {
